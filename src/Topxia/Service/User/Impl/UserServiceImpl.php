...
{   
    ...
    public function oldVerifyInSaltOut($in,$salt,$out)
    {
    ...
    public function verifyPayPassword($id, $payPassword)
    {
        ...
        return $this->oldVerifyInSaltOut($payPassword, $user['payPasswordSalt'], $user['payPassword']);
    }
    ...
    */
    ...
    private function sendRegister($user) 
    {
        $time = time();
        $url = "http://sso.ciptc.org.cn/apps/inner/register.shtml?curtime={$time}";
        $data = [
            'iloginname' => $user['nickname'],
            'ipassword' => $user['password'],
            'iemail' => $user['email'],
            'irealname' => '待填', 
            'isalt' => $user['salt'],
            'iregip' =>  $user['createdIp'],
            'iregtime' => $user['createdTime'],  
        ];
        ...
        $this->getLogService()->info('user', 'send_register', "用户".$user['nickname']."注册同步失败");
        return false;
    }

    public function sendUpdateUser($user) 
    {   
        $user = $this -> getUserByEmail($email);
        $time = time();
        $url = "http://sso.ciptc.org.cn/apps/inner/memmod.shtml?curtime={$time}";
        $data = [
            'iloginname' => $user['nickname'],
            'ipassword' => $user['password'],
            'iuid' => $user['id'],
            'isalt' => $user['isalt'],
            'iemailstatus' => $user['emailVerified'],
            'iemail' => $user['email'],
        ];

        if (isset($user['truename'])) {

            $data['irealname'] = $user['truename'];
        }
        if (isset($user['gender'])) {

            $data['igender'] = $user['gender'];
        }
        ...
        $this->getLogService()->info('user', 'send_userinfo', "用户".$user['nickname']."同步更新用户信息失败");
        return false;
    }

    public function sendLogin($email) 
    {   
        $user = $this -> getUserByEmail($email);
        $time = time();
        $url = "http://sso.ciptc.org.cn/apps/mem/login.shtml?curtime={$time}";
        $data = [
            'user' => $user['nickname'],
        ...
        $this->getLogService()->info('user', 'send_login', "用户".$user['nickname']."同步登录失败");
        return false;
    }
    ...
    public function updateUserProfile($id, $fields)
    {
        ...
        $user['truename'] = $fields['truename'];

        if($fileIds['gender'] == "male") {
    ...
    public function addCode($code, $id)
    {
        ...
        return $this->getUserDao()->updateUser($user['id'], array('code' => $code));
    }
    ...
    public function activeCode($id)
    {
        ...
        return $this->getUserDao()->updateUser($user['id'], array('activeCode' => 1));
    }
